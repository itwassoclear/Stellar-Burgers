...
import { SHOW_DETAILS, CLOSE_DETAILS, getDetails } from '../../services/actions/index';
    ...
    <section className={styles.box}>
      ...
      <div className={styles.ingridients} onScroll={scrollToBlock}>
        ...
        <div className={clsx(styles.items, "pl-4 pr-4")}>
          {Object.values(buns).map(elem => <Ingredient elem={elem} key={elem._id} handleOpenModal={handleOpenModal} />)}
        </div>

        <p className="text text_type_main-medium mt-10 pb-6">Соусы</p>
        <div className={clsx(styles.items, "pl-4 pr-4")}>
          {Object.values(sauces).map(elem => <Ingredient elem={elem} key={elem._id} handleOpenModal={handleOpenModal} />)}
        </div>

        <p className="text text_type_main-medium mt-10 pb-6">Начинки</p>
        <div className={clsx(styles.items, "pl-4 pr-4")}>
          {Object.values(mains).map(elem => <Ingredient elem={elem} key={elem._id} handleOpenModal={handleOpenModal} />)}
        </div>
